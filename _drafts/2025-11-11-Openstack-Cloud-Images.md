...
Enter ["Cloud Images"](https://docs.openstack.org/image-guide/obtain-images.html). These OS images are basically instances prepared/curated by the different Linux distribution projects (or companies) ready to boot in a cloud environment (AWS, Azure, GCP, Openstack, etc). You can treat them as secure as the ISO installer you download for your favorite distribution project, they include a minimal footprint and usually have cloud-init already pre-installed to automate deployment.

In Openstack, cloud-init is critical for deployment as you don't want to resize your boot disk, add users or setup TCP/IP by hand. Also QEMU Guest Agent is highly recommended when virtualizing on KVM to enable consistent backups.

Paid Linux distributions also provide cloud images (SLES/RHEL), I can't seem to find a Windows Server 2025 Cloud image to download (shame on you Microsoft!)
...
If you're using a filesystem backend, go with QCOW2. If you're using Ceph RBD like me, go with RAW in order to benefit from Copy-on-Write cloning of images in both Cinder and Nova & quicker instantiation times.
